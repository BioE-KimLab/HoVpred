...
def dgl_molgraph(data, Max_atoms, device, explicit_Hs = False):
    ...
    #g = dgl.graph((src_ids, dst_ids), num_nodes = len(data) * Max_atoms)
    g = dgl.graph((src_ids, dst_ids), num_nodes = len(data) * Max_atoms).to(device)
    #g = dgl.graph((src_ids, dst_ids), num_nodes = len(data) * Max_atoms).to("/gpu:0")
    g.ndata['feat'] = all_atom_features
    #g = dgl.remove_self_loop(g)
    #g = dgl.add_self_loop(g)

    #g.edata['feat'] = bond_features   #we can add some if necessary in the future
    return g
...
def single_mol_from_supergraph(Graphs, H, Attention, Max_atoms):
    ...
    print(A_per_each_mol)
    print(src_per_each_mol)
    print(dst_per_each_mol)

def glob_features(smi):
    mol = rdkit.Chem.MolFromSmiles(smi)
    return np.array([ Chi1(mol), Ipc(mol), sum(EStateIndices(mol)), CalcNumRings(mol), CalcNumHBD(mol)   ])
    #return np.array([ Chi1(mol), sum(EStateIndices(mol)), CalcNumRings(mol), CalcNumHBD(mol)   ])
    #return np.array([ Chi1(mol), CalcNumRings(mol), CalcNumHBD(mol)   ])
